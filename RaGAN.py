# -*- coding: utf-8 -*-
"""
Created on Fri Dec 14 20:49:06 2018
...
os.makedirs('images_RaGAN', exist_ok=True)
...
for epoch in range(opt.n_epochs):
    for i, (imgs, _) in enumerate(dataloader):
        ...
        data1 = d_x - torch.mean(d_g_z)
        data2 = d_g_z - torch.mean(d_x)

        # Loss measures generator's ability to fool the discriminator
        g1_loss = adversarial_loss(data1, valid)
        g2_loss = adversarial_loss(data2, fake)
        ...
        d1_loss = adversarial_loss(data1, fake)
        d2_loss = adversarial_loss(data2, valid)
        ...
        if batches_done % opt.sample_interval == 0:
            save_image(g_z.data[:25], 'images_RaGAN/%d.png' % batches_done, nrow=5, normalize=True)
